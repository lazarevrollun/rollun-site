...
- **Переписать всё на Laravel / другой стек** — отклонён для этого проекта. Laravel как PHP-бэкенд не даёт пиксель-в-пиксель React-страницы из коробки (фронт всё равно строить на Blade/Livewire/Inertia), выбрасывает ключевое решение «захардкоженный layout + типизированные слоты», ломает срок «пара дней» и не приближает CRM (готовую CRM одинаково просто интегрировать из Next.js). Он был бы уместен для отдельного большого проекта — собственного бэк-офиса или кастомной CRM с горизонтом в месяцы, а не имиджевого сайта на дни.
