...
- **Mobile — letterbox-шелл.** Мобильная композиция живёт в центрированном «телефоне» шириной `{spacing.shell-mobile}` (=440px, `.phone{max-width:440px;margin:0 auto}`); контент внутри — на `{colors.bg}` (=`#D2D2D2`), а боковые letterbox-поля вне шелла — на `{colors.mobile-shell-bg}` (=`#0c0c0c`). Полоса 440–768px — mobile-шелл в letterbox, не desktop.
